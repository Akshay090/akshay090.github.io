...
In this post i'm going to introduce one of the concepts you can use when developing web pages/apps using [Theme UI](https://theme-ui.com/)

If you're not familiar with Theme UI take a moment to have a read of the docs... and if you're not familiar with [CSS-in-Js](https://medium.com/dailyjs/what-is-actually-css-in-js-f2f529a2757) perhaps have a read up on that first.
...
If you'd like to know about more, have a read of this: [What is JSX Pragma](https://www.gatsbyjs.org/blog/2019-08-02-what-is-jsx-pragma/)
...
You can see from the above that the CSS for the `<Box />` includes some resets for us, eg. `box-sizing`,`margin` and `min-width` and whilst in the past we _could_ have used a global CSS file that handles the resets, we don't want to use global CSS because this is where problems arise.
...
The `as` props works with all manner of HTML elements... even inputs <Box as='input' placeholder="I'm an input"/>
