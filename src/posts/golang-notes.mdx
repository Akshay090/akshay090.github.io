...
These are the notes I made while doing the [`Coursera Golang Specialization`](https://www.coursera.org/specializations/google-golang)
for quick reference.
...
Stack: variables defined within a function - Memory will be deallocated when function call completed (in C for example)
Heap: Global variables - Needs to be explicitly deallocated ( in C for example), is fast but error-prone
